...
class SEALDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # random walk
        split_obsrv = True if self.split == 'train' and self.percent < 100 else False
        #split_obsrv = False
        if self.use_rpe:
            A_obsrv = self.A.copy()
            if split_obsrv:
...
def main():
    ...
    data.timestamps = torch.randint(low=0, high=num_nodes, size=(num_nodes, 1))

    split_edge = do_edge_split([data])
    data.edge_index = split_edge['train']['edge'].t()
    root = os.path.dirname(os.path.abspath(__file__))
    preprocess_fn = partial(preprocess.preprocess, 
                       use_len_spd=True,
                       use_num_spd=True,
                       use_cnb_jac=True,
                       use_cnb_aa=True,
                       use_cnb_ra=True)
    dataset = SEALDynamicDataset(root, data, split_edge, 1, directed=True, preprocess_fn = preprocess_fn)
    print('Making mini graph')
    subgraph = dataset.get(0)
    print(subgraph)
